...
import { useCreateExpensiveProduct } from "@/apis/hooks/useExpensiveProducts";

interface Tag { id: number; label: string; }
interface AddExpensiveProductProps { onClose: () => void; }
...
export default function AddExpensiveProduct({ onClose }: AddExpensiveProductProps) {
    ...
    const { mutate: createProduct, isPending } = useCreateExpensiveProduct();
    ...
    const handleSave = () => {
        // ✅ Use tags if they exist, otherwise fall back to raw input
        ...
        if (!brand.trim() || !productName.trim() || !price.trim() || !finalSearchTerms || !finalKeyActives) {
            Swal.fire({
        ...
        createProduct(
            ...
            {
                onSuccess: (data) => {
                    //  console.log("onSuccess Expensive data:", data);
                    Swal.fire({
                        title: "Product Added!",
                        text: "Expensive product has been saved.",
                        icon: "success",
                        confirmButtonColor: "#2d5a3d",
                        customClass: { popup: "rounded-2xl", confirmButton: "rounded-lg text-sm font-medium" },
                    });
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 relative" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="px-8 pt-7 pb-5 border-b border-gray-100">
                    <button onClick={onClose}
                        className="cursor-pointer absolute top-5 right-5 w-7 h-7 flex items-center justify-center rounded-full bg-gray-900 text-white text-sm hover:bg-gray-700 transition-colors">
                        ✕
                    </button>
                ...
                <div className="px-8 py-5 border-t border-gray-100 flex justify-end gap-3">
                    <button onClick={onClose}
                        className="cursor-pointer px-6 py-2.5 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                        Cancel
                    </button>
