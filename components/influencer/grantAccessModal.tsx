"use client";
import { useGrantInfluencerAccess } from "@/apis/hooks/useInfluencers";
...
export default function GrantInfluencerAccessModal({ onClose }: { onClose: () => void }) {
  ...
  const { mutate: grantAccess, isPending } = useGrantInfluencerAccess();
  ...
  const handleSubmit = () => {
    ...
    grantAccess(
      { email, note, is_active: true },
      {
        onSuccess: () => {
          Swal.fire({
            title: "Access Granted!",
            text: `${email} now has influencer access.`,
            icon: "success",
            confirmButtonColor: "#2D6A4F",
            customClass: {
              popup: "rounded-2xl",
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl w-[50vw] max-w-none overflow-hidden">
      ...
      <div className="flex items-start justify-between px-8 pt-7 pb-5 border-b border-gray-100">
        ...
        <button
          onClick={onClose}
          className="w-8 h-8 bg-gray-900 rounded-full flex items-center justify-center text-white hover:bg-gray-700 transition-colors shrink-0 mt-0.5"
        >
      ...
      <div className="flex items-center justify-end gap-3 px-8 py-5 border-t border-gray-100">
        ...
        <button
          ...
          disabled={isPending}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold text-white bg-green-800 hover:bg-green-700 transition-colors disabled:opacity-60 cursor-pointer"
        >
          {isPending && (
